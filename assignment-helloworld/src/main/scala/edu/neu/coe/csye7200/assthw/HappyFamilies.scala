...
case class Family(father: Person, mother: Person, children: Seq[Person]) {
  /**
    ...
    * @return a new Family.
    */
  // def birth(child: Person): Family = Family(father, mother, children = Seq(child))// TODO (10) implement
  def birth(child: Person): Family = Family(this.father, this.mother , this.children :+ child)
}
...
case class Person(name: String, gender: Gender, age: Option[Int]) {
  ...
  def marry(person: Person): Try[Family] = gender.join(person.gender) match {
    ...
    //    case person.gender == "Male" & this.gender == "Female" => Try(Family(person, this, Seq(null)))
    //    case person.gender == "Female" & this.gender =="Male" => Try(Family(this, person,Seq(null)))
    //    case person.gender == "Female" & this.gender == "Female" => Failure(GenderException("Illegal"))
    //    case person.gender != this.gender => Try(Family(this, person,Seq(null)))
    //    case person.gender == this.gender => Failure(GenderException("Illegal"))
...
case class Gender(maybeMale: Option[Boolean]) {
  ...
  // ???
    for (x <- maybeMale; y <- other.maybeMale) yield (x, y) match {
